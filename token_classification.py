...
def main():
    ...
    def tokenize_and_align_labels(examples):
        ...
        labels = []
        # print(f'\nprinting----------------------{examples[data_args.tags_key]}')
        for i, label in enumerate(examples[data_args.tags_key]):
            ...
            for word_idx in word_ids:
                if word_idx is None:
                    ...
                    # print(f'\n\nword_idx: {word_idx}, label[word_idx]: {label[word_idx]}, label_to_id[label[word_idx]]: {label_to_id[label[word_idx]]}\n\n') 
    ...
    if training_args.do_predict:
        ...
        output_predictions_file_reformat = os.path.join(training_args.output_dir, "predictions_reformed.txt")
        if trainer.is_world_process_zero():
            with open(output_predictions_file, "w") as writer:
                for prediction in true_predictions:
                    writer.write(" ".join(prediction) + "\n")
            
            with open(output_predictions_file, "r") as reader:
                for line in reader:
                    words = line.split(' ')
                    with open(output_predictions_file_reformat, "a") as writer:
                        for word in words:
                            writer.write(word + '\n')
            with open(output_predictions_file_reformat, "r+") as f:
                f.seek(0, 2)
                size = f.tell()
                f.seek(max(0, size - 2), 0)
                f.truncate()
